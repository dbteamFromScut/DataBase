...
public class AcdemicController {
    ...
    @ResponseBody
    public void exportTeacher(HttpServletResponse response) throws UnsupportedEncodingException {
        List<Teacher> teachers=teacherDao.selectByPrimaryKeyList();
        Workbook workbook = ExcelExportUtil.exportExcel(new ExportParams(),
                Teacher.class, teachers);
        response.setContentType("application/vnd.ms-excel;charset=utf-8");
        response.setCharacterEncoding("utf-8");
        // 对文件名进行处理。防止文件名乱码
        String fileName = URLEncoder.encode("教师信息.xls","UTF-8");
